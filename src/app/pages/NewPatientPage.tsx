...
import { ArrowLeft, User, RefreshCw, X, AlertTriangle, CheckCircle, ChevronDown, ChevronRight } from 'lucide-react';
  ...
  const handleSave = async () => {
    try {
      const p = await createPatient({
        ...
        referredBy: formData.referredBy, allergies, insuranceProvider: formData.insuranceProvider,
        insurancePolicy: formData.insurancePolicy, notes: formData.notes,
        isActive: true, visitCount: 0
      } as any, "user");
  ...
  return (
    <div>
      <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:22 }}>
        <button onClick={() => navigate("/patients")} style={{ background:"none", border:"none", cursor:"pointer", color:P.textMuted, display:"flex", padding:4 }}><ArrowLeft size={18}/></button>
        <h1 style={{ fontSize:18, fontWeight:500, color:P.textPrimary }}>New patient registration</h1>
      </div>
      <div style={{ display:"grid", gridTemplateColumns:"200px 1fr", gap:22, maxWidth:960 }}>
        <div>
          <Card style={{ padding:20, textAlign:"center" }}>
            <div style={{ width:96, height:96, borderRadius:12, border:`2px dashed ${P.border}`, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", cursor:"pointer", margin:"0 auto 12px", background:P.bgSunken }}>
              <User size={28} color={P.textMuted}/><span style={{ fontSize:10, color:P.textMuted, marginTop:4 }}>Add photo</span>
            </div>
            <div style={{ fontSize:11, color:P.textMuted, marginBottom:16 }}>Webcam or upload</div>
            <div style={{ textAlign:"left", marginBottom:12 }}>
              <div style={{ fontSize:10, fontWeight:500, color:P.textSecondary, letterSpacing:"0.04em", marginBottom:4 }}>Patient ref. ID</div>
              <div style={{ fontFamily:"JetBrains Mono, monospace", fontSize:14, fontWeight:500, color:P.textPrimary, background:P.bgSunken, border:`1px solid ${P.border}`, borderRadius:6, padding:"7px 10px", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
                Auto-generated<RefreshCw size={12} color={P.textMuted} style={{ cursor:"pointer" }}/>
              </div>
            </div>
          </Card>
        </div>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
                ...
                <div>
                  ...
                  <div style={{ display:"flex", gap:6 }}>
                    {(["MALE","FEMALE","OTHER"] as const).map(s => <button key={s} onClick={() => setSex(s)} style={{ flex:1, height:36, borderRadius:8, border:`1.5px solid ${sex===s?P.violet:P.border}`, background:sex===s?P.violetLight:P.bgSunken, color:sex===s?P.violetDark:P.textSecondary, fontSize:12, fontWeight:500, cursor:"pointer", fontFamily:"inherit", transition:"all 150ms" }}>{s}</button>)}
                  </div>
                </div>
              </div>
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
            ...
            <div style={{ padding:20, display:"flex", flexDirection:"column", gap:14 }}>
              ...
              <div>
                ...
                <div style={{ display:"flex", flexWrap:"wrap", gap:6, marginBottom:8 }}>
                  {allergies.map(a => <div key={a} style={{ display:"inline-flex", alignItems:"center", gap:4, background:P.siennaLight, color:P.sienna, padding:"2px 8px", borderRadius:999, fontSize:11, fontWeight:500 }}>{a}<button onClick={() => setAllergies(allergies.filter(x => x!==a))} style={{ background:"none", border:"none", cursor:"pointer", color:P.sienna, display:"flex", padding:0 }}><X size={10}/></button></div>)}
                </div>
                ...
                {allergies.length>0&&<div style={{ marginTop:10, padding:"8px 12px", borderRadius:8, background:P.siennaLight, border:`1px solid ${P.sienna}`, fontSize:12, color:P.sienna, display:"flex", alignItems:"center", gap:6 }}><AlertTriangle size={13}/>Allergy on file — will appear on all visit and prescription screens</div>}
              </div>
              ...
              <div>
                <button onClick={() => setShowIns(!showIns)} style={{ background:"none", border:"none", cursor:"pointer", fontSize:13, color:P.violet, fontFamily:"inherit", display:"flex", alignItems:"center", gap:4 }}>
                  {showIns?<ChevronDown size={14}/>:<ChevronRight size={14}/>}Add insurance details
                </button>
                {showIns&&<div style={{ marginTop:12, display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}><Inp label="Insurance provider" value={formData.insuranceProvider} onChange={v => handleChange('insuranceProvider',v)} placeholder="Provider name"/><Inp label="Policy number" value={formData.insurancePolicy} onChange={v => handleChange('insurancePolicy',v)} placeholder="POL-XXXXXXXX"/></div>}
              </div>
            </div>
          </Card>
        </div>
      ...
      <div style={{ position:"sticky", bottom:0, background:P.bgBase, borderTop:`1px solid ${P.border}`, padding:"12px 0", marginTop:20, display:"flex", justifyContent:"flex-end", gap:10 }}>
        <Btn variant="ghost" onClick={() => navigate("/patients")}>Discard</Btn>
        <Btn variant="primary" onClick={handleSave} icon={<CheckCircle size={14}/>}>Save and open profile</Btn>
      </div>
    </div>
  );
}
